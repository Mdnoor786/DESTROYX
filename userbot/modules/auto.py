...
from userbot import BIO_MESSAGE, BIO_MSG, CMD_HELP, JAVES_NAME, bot
...
async def _(event):
    while True:
        ...
        nnmel = f"{HM}"
        await event.reply(
            f"**{JAVES_NAME}**: `successfully set last name to {HM} {TZZ}\n Sleeping 60s........`"
        )
...
async def _(event):
    while True:
        ...
        bio = f" {BIO_MMSG}| {HM}"
        await event.reply(
            f"**{JAVES_NAME}**: `successfully set auto bio with {BIO_MMSG} with {HM} {TZZ}\n Sleeping 60s.......`"
        )
...
async def _(event):
    ...
    while True:
        ...
        file = await bot.upload_file(photo)  # pylint:disable=E0602
        await event.reply(
            f"**{JAVES_NAME}**: `successfully set profile picture \nSleeping 60s.......`"
        )
        try:
            await bot(
                functions.photos.UploadProfilePhotoRequest(file)  # pylint:disable=E0602
