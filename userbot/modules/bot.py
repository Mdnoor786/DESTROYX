...
async def _(event):
    ...
    try:
        ...
        async with event.client.conversation(chat, timeout=7) as conv:
            try:
                test = conv.wait_event(
                    events.MessageEdited(incoming=True, from_users=232268607)
                )
    ...
    try:
        ...
        async with event.client.conversation(chat, timeout=7) as conv:
            try:
                ...
                return await rkp.edit(
                    "`Please unblock @Stickerdownloadbot and try again`"
                )
...
async def _(event):
    ...
    try:
        ...
        async with event.client.conversation(chat, timeout=7) as conv:
            try:
                test = conv.wait_event(
                    events.NewMessage(incoming=True, from_users=355705793)
                )
                await event.client.send_message(chat, reply_message)
                response = await test
            except YouBlockedUserError:
