...
from userbot.events import bot, javes05, rekcah05
...
async def evaluate(event):
    ...
    if cmd in ("userbot.session", "env", "printenv"):
        return await event.edit(
            f"`{JAVES_NNAME}:` **Privacy Error, This command not permitted**"
        )
    ...
    if (stdout or stderr) and len(text) <= 4096:
        return await event.edit(text)
    with open("term.txt", "w+") as output:
        output.write(text)
...
@javes.on(rekcah05(pattern="term(?: |$|\\n)([\\s\\S]*)", allow_sudo=True))
async def evaluate(event):
    if not FULL_SUDO:
        await query.reply(
...
async def run(run_q):
    ...
    if not code:
        await run_q.edit(
            f"`{JAVES_NNAME}:` **At least a variable is required to \
...
@javes.on(rekcah05(pattern="exec(?: |$)([\\s\\S]*)", allow_sudo=True))
async def run(run_q):
    if not FULL_SUDO:
        await query.reply(
            f"`{JAVES_NNAME}:` **Sorry , Normal Sudo cant acess this comand,  active advance sudo by set  FULL_SUDO as true in heroku var**"
...
async def evaluate(query):
    if query.pattern_match.group(1):
        ...
        await query.edit(
            f"`{JAVES_NNAME}:` **Please give command type !help script for more info**"
        )
        return
    ...
    try:
        if evaluation := str(eval(expression)):
...
@javes.on(rekcah05(pattern="eval(?: |$)(.*)", allow_sudo=True))
async def evaluate(query):
    if not FULL_SUDO:
        await query.reply(
            f"`{JAVES_NNAME}:` **Sorry , Normal Sudo cant acess this comand,  active advance sudo by set  FULL_SUDO as true in heroku var**"
...
@javes.on(rekcah05(pattern="hash (.*)", allow_sudo=True))
async def gethash(hash_q):
    hashtxt_ = hash_q.pattern_match.group(1)
    with open("hashdis.txt", "w+") as hashtxt:
        hashtxt.write(hashtxt_)
...
@javes.on(rekcah05(pattern="base64 (en|de) (.*)", allow_sudo=True))
async def endecrypt(query):
    if query.pattern_match.group(1) == "en":
        lething = str(pybase64.b64encode(bytes(query.pattern_match.group(2), "utf-8")))[
            2:
