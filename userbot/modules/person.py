...
javes = bot
UNAPPROVED_MSG = f"`{JAVES_NNAME}:`**{PM_MESSAGE}**"
        ...
        if PM_AUTO_BAN:
            ...
            if (
                ...
                if not apprv and event.text != UNAPPROVED_MSG:
                    ...
                    if COUNT_PM[event.chat_id] > MAX_PM:
                        await event.respond(f"`{JAVES_NNAME}`: ** {BLOCK_MMSG} **")
                        try:
...
async def unblockpm(unblock):
    ...
    if BOTLOG:
        await unblock.client.send_message(
            BOTLOG_CHATID,
            f"[{name0}](tg://user?id={ruser.id})" " was unblocked!.",
        )
...
async def afk_on_pm(sender):
    ...
    if ISAFK:
        ...
        if (
            sender.is_private
            and sender.sender_id != 710844948
...
async def _(event):
    ...
    ruser_profile_photos = await event.client(
        GetUserPhotosRequest(user_id=ruser.user.id, offset=42, max_id=0, limit=80)
    )
...
""".format(
        ...
        user_id,
        user_id,
        ruser.user.first_name,
        ruser.user.last_name,
        ruser.user.username,
        user_bio,
...
async def get_user(event):
    if event.reply_to_msg_id and not event.pattern_match.group(1):
        ...
        try:
            ...
            ruser = await event.client(GetFullUserRequest(user_object.id))
...
@javes.on(rekcah05(pattern="purge$", allow_sudo=True))
