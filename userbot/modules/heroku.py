...
from userbot import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, JAVES_NAME
from userbot.events import javes05
from userbot.events import rekcah05 as admin_cmd
...
@javes.on(
    admin_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\\s\\S]*)", allow_sudo=True)
)
async def variable(var):
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
...
CMD_HELP.update(
    {
        "heroku": "** heroku apikey, name must set correctly for use this commands get help from https://telegra.ph/RkPavi-06-09-6** "
        "\n\n`!usage`"
