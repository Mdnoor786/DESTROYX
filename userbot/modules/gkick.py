...
from userbot import CMD_HELP, JAVES_MSG, JAVES_NAME, bot
...
async def gspide(rk):
    ...
    if user:
        ...
        for i in testrk:
            try:
                ...
                a += 1
                await rkp.edit(
                    f"`{JAVES_NNAME}:` **Requesting  to gkicking user!\nGkicked {a} chats.....**"
                )
    ...
    return await rkp.edit(
        f"`{JAVES_NNAME}:` **GKicked [{user.first_name}](tg://user?id={user.id}) in {a} chat(s) **"
    )
...
CMD_HELP.update(
    {
        "gkick": "!gkick/.gkick <username> / <userid> / <reply to a user>\
