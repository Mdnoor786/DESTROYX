...
from userbot import CMD_HELP, CMD_LIST, client
from userbot.events import javes05, load_module, remove_plugin, zzaacckkyy
...
async def install(event):
    try:
        ...
        cui = await client.get_messages(
            chat, limit=75, filter=InputMessagesFilterDocument
        )
        total = int(cui.total)
        ...
        if "(" not in downloaded_file_name:
            ...
            if (os.path.basename(downloaded_file_name)).endswith(".py"):
                try:
                    load_module(shortname.replace(".py", ""))
                    text += (
                        f"**• Installed** {(os.path.basename(downloaded_file_name))}\n"
                    )
                except:
...
CMD_HELP.update(
    {
        "install": "`!install <reply to a plugin>`\
