...
async def let_me_google_that_for_you(lmgtfy_q):
    sender = await lmgtfy_q.get_sender()
    me = await lmgtfy_q.client.get_me()
    if not sender.id == me.id:
    ...
    if len(input_sgra) == 3:
        try:
            ...
            if currency_to in current_response["rates"]:
                ...
                await rkp.edit("`unknown currency, which I can't convert right now.`")
...
async def gsearch(q_event):
    sender = await q_event.get_sender()
    me = await q_event.client.get_me()
    if not sender.id == me.id:
...
async def _(event):
    ...
    rkkk = await event.reply(
        f"`{JAVES_NNAME}: ` **Finding {rksong} in world wide please wait........**"
    )
    chat = "@FindMusicPleaseBot"
    async with event.client.conversation(chat) as conv:
        response = conv.wait_event(
            events.NewMessage(incoming=True, from_users=442186886)
        )
        await conv.send_message("/start")
        response = await conv.get_response()
        if not response.text.startswith("Hi,"):
            await event.reply(
                f"`{JAVES_NNAME}`: **Sorry, look like my frined is sleeping.... please try later \nOr use alternative way !song <song name> **"
            )
        else:
            ...
            if song2.text.startswith("Sorry,"):
                await rkkk.edit(
                    f"`{JAVES_NNAME}`: `Sorry,failed to find \nTry alternative way !song <songname>`"
                )
            else:
