...
async def _(e):
    await e.edit("HELP MENU CLOSED", buttons=[[Button.inline("ReOPeN", data="back")]])
    ...
    async def ban(event):
        if not BT:
            return await event.edit(
                " Error Add bot token as BOT_TOKEN in heroku var or set inline mode on "
            )
        ...
        if os.path.exists("./userbot/secrets.txt"):
            ...
            try:
                ...
                if event.query.user_id in ids:
                    ...
                    reply_pop_up_alert = (
                        "why were you looking at this  go away and do your own work"
                    )
            except KeyError:
        ...
        try:
            ...
            if not event.query.user_id == me.id:
                return await event.answer(
                    "Sorry, You dont have permission to  Access me!", alert=True
                )
            ...
            if et == "back":
                ...
                return await event.edit(
                    "Support Group For Report bugs & help @JavesGrouP",
                    buttons=tbu,
                    link_preview=False,
                )
            if et in CMD_HELP:
...
if tebot:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("helpme"):
            ...
            result = builder.article(
                "Help menu",
                text="Support Group For Report bugs & help @javes2support",
                buttons=tbu,
                link_preview=False,
            )
