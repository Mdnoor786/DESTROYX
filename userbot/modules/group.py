...
from userbot import BOTLOG, BOTLOG_CHATID, JAVES_MSG, JAVES_NAME, bot
from userbot.events import javes05, rekcah05
from userbot.modules.sql_helper.rkfilter_sql import (
    add_filter,
    get_all_rkfilters,
    remove_all_rkfilters,
    remove_filter,
)
...
CHAT_PP_ERROR = (
    f"`{JAVES_NNAME}:`**Some issue with updating the pic,**"
...
async def log(log_text):
    ...
    if BOTLOG:
        ...
        await log_text.edit(
            f"`{JAVES_NNAME}`: **This feature requires Logging to be enabled!**"
        )
    await sleep(2)
...
async def iqless(e):
    await e.reply(
        f"`{JAVES_NNAME}`: **Privacy error! , Sorry sudo users dont have permission to access it!**"
    )
...
async def kickme(leave):
    ...
    await leave.edit(
        f"`{JAVES_NNAME}`: **My master Didnt like this place......GoodBye!**"
    )
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                await show.edit(
                    f"`{JAVES_NNAME}:` **Sorry, I don't have ban rights in this group"
                )
                return
...
async def remove_a_filter(r_handler):
    ...
    filt = r_handler.pattern_match.group(1)
    if not remove_filter(r_handler.chat_id, filt):
        await r_handler.reply(f"`Filter` **{filt}** `doesn't exist.`")
    else:
        await r_handler.reply(f"`Filter` **{filt}** `was deleted successfully`")
...
async def _(event):
    ...
    if event.is_private:
        await event.edit(
            f"**{JAVES_NNAME}:** invite  users to a chat, not to a Private Message"
        )
    else:
        ...
        if not event.is_channel and event.is_group:
            ...
            for user_id in to_add_users.split(" "):
                try:
                    ...
                    await event.reply(str(e))
            await event.reply(
                f"**{JAVES_NNAME}:** Invite request sent telethon Successfully"
            )
        else:
            # https://lonamiwebs.github.io/Telethon/methods/channels/invite_to_channel.html
            for user_id in to_add_users.split(" "):
...
async def on_snip_save(event):
    ...
    if msg:
        ...
        await event.edit(
            f"`{JAVES_NNAME}`: filter {name} saved successfully. Get it with {name}"
        )
    else:
        await event.edit(
            f"`{JAVES_NNAME}`: **Reply to a message with `!savefilter keyword` to save the filter**"
        )
...
async def on_all_snip_delete(event):
    ...
    await event.edit(
        f"`{JAVES_NNAME}`: filters **in current chat** deleted successfully"
    )
